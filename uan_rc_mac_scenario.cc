...
using namespace ns3;
using namespace std;
NS_LOG_COMPONENT_DEFINE ("uan_rc_mac_scenario");
AnimationInterface *pAnim;
double ds=1000.0;  
int rounds=300;      
uint32_t m_pktSize=1000; 
Time m_sifs(Seconds (0.05));       
Time m_simTime(Seconds (5000)); 
uint32_t m_bytesTotal=0; 
bool m_doNode=true; 
uint32_t m_numNodes;
uint32_t m_simMin=1;   
uint32_t m_simMax=1;   
uint32_t m_simStep=1;  
uint32_t m_numRates=1023;
uint32_t m_totalRate=4096; 
uint32_t m_maxRange=3000; 
uint32_t param=10;
uint32_t fc=10;
...
std::cout<<"\n  we make a communication between the sensor node and server node through the sink node based on the uan-rc-mac protocol process. \n\n";
for(  uint32_t i=0;i<c.GetN ();i++){
TypeId tid1 = TypeId::LookupByName ("ns3::UdpSocketFactory");
Ptr<Socket> recvSink1 = Socket::CreateSocket (d.Get (0), tid1);
InetSocketAddress local1 = InetSocketAddress (Ipv4Address::GetAny (), 80);
...
for (uint32_t i=1; i < m_numRates+1; i++){m_controlModes.AppendMode (CreateMode (i, fc, false, "control "));}
for (uint32_t i=m_numRates; i > 0; i--){m_dataModes.AppendMode (CreateMode (i, fc, true, "data "));}
...
uan.SetPhy ("ns3::UanPhyDual","SupportedModesPhy1", UanModesListValue (m_dataModes),"SupportedModesPhy2", UanModesListValue (m_controlModes));
uan.SetMac ("ns3::UanMacRcGw","NumberOfRates", UintegerValue (m_numRates),"NumberOfNodes", UintegerValue (nNodes),"MaxReservations",UintegerValue (a),"SIFS", TimeValue (m_sifs),"MaxPropDelay", TimeValue (pDelay),"FrameSize", UintegerValue (m_pktSize));
...
macgplot mg;
mg.Packet_Delivery_Ratio(noOfNodes,"RC");
mg.Energy_Consumption(noOfNodes,"RC");
mg.Delay(noOfNodes,"RC");
mg.Throughput(noOfNodes,"RC");
pAnim= new AnimationInterface ("uan_rc_mac_scenario.xml");

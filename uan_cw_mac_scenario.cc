...
using namespace ns3;
using namespace std;
NS_LOG_COMPONENT_DEFINE ("uan_cw_mac_scenario");
...
std::cout<<"\n  we make a communication between the sensor node and server node through the sink node based on the uan-cw-mac protocol process.  \n\n";
for(  uint32_t i=0;i<c.GetN ();i++){
TypeId tid1 = TypeId::LookupByName ("ns3::UdpSocketFactory");
Ptr<Socket> recvSink1 = Socket::CreateSocket (d.Get (0), tid1);
InetSocketAddress local1 = InetSocketAddress (Ipv4Address::GetAny (), 80);
...
uint32_t m_cwMin=10;                  
uint32_t m_cwMax=400;                 
uint32_t m_cwStep=10;                 
std::string m_gnudatfile="uan-cw-example.gpl";
std::string m_asciitracefile="uan-cw-example.asc"; 
...
uan.SetMac ("ns3::UanMacCw", "CW", UintegerValue (m_cwMin), "SlotTime", TimeValue (Seconds (simtime)));
...
source->Connect (remote);
for (uint32_t cw = m_cwMin; cw <= m_cwMax; cw += m_cwStep){Simulator::Schedule (Seconds (1.3), &GenerateTraffic, source, packetSize, noofpkts, interPacketInterval);}
...
macgplot mg;
mg.Packet_Delivery_Ratio(noOfNodes,"CW");
mg.Energy_Consumption(noOfNodes,"CW");
mg.Delay(noOfNodes,"CW");
mg.Throughput(noOfNodes,"CW");
pAnim= new AnimationInterface ("uan_cw_mac_scenario.xml");
